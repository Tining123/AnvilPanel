...
public class AdminPanelDeleteGUI {
    private static final Logger log = Logger.getLogger("Minecraft");
    /**
     * 当前开启的菜单
     */
    private static final Map<UUID, AdminPanelDeleteGUI> MENU_OPENING = new ConcurrentHashMap();
     ...
     * @param player
     */
    public AdminPanelDeleteGUI(Player player, Panel panel) {
    ...
    private static void drawPage(Inventory inventory, Player player) {
        ...
        // 设置功能性图标
        PluginUtil.setSign(inventory, AdminPanelDeleteGUI.SignEnum.GROUP, SignMaterialEnum.LABEL, panel.getName() , new ArrayList<>());
        PluginUtil.setSign(inventory, AdminPanelDeleteGUI.SignEnum.CONFIRM, SignMaterialEnum.ON, LangReader.get("确认"), new ArrayList<>());
        PluginUtil.setSign(inventory, AdminPanelDeleteGUI.SignEnum.CANCEL, SignMaterialEnum.OFF, LangReader.get("取消"), new ArrayList<>());
    }
    ...
    public enum SignEnum implements PublicSignEnumInterface {
        ...
        CONFIRM(41, "确认") {
            ...
            public void deal(Inventory inventory, Player player) {
                Panel panel = MENU_OPENING.get(player.getUniqueId()).panel;
                PanelReader.getInstance().delete(panel);
                player.closeInventory();
                unRegister(player);
                player.sendMessage(LangReader.get("删除成功"));
            }
        },
        CANCEL(39, "取消") {
            @Override
         ...
         */
        public static AdminPanelDeleteGUI.SignEnum findMatchedSign(int checkSlot) {
            for (AdminPanelDeleteGUI.SignEnum sign : AdminPanelDeleteGUI.SignEnum.values()) {

 ...
 * @author tinga
 */
public class AdminGroupUserListGUI {
    private static final Logger log = Logger.getLogger("Minecraft");
    /**
     * 当前开启的菜单
     */
    private static final Map<UUID, AdminGroupUserListGUI> MENU_OPENING = new ConcurrentHashMap();
     ...
     * 收购列表名称
     */
    public static final String GUI_NAME = "管理员用户组用户展示列表";
     ...
     * @param player
     */
    public AdminGroupUserListGUI(Player player, Group group) {
    ...
    private static void drawPage(Inventory inventory, int pageNum, Player player) {
        int move = 0;
        boolean set = false;
        Group group = MENU_OPENING.get(player.getUniqueId()).group;
        ...
        String selectItem = AdminGroupUserListGUI.MENU_OPENING.get(player.getUniqueId()).selectItem;
        ...
        if (!Objects.isNull(leftItemMeta) && pageNum != 0 && pageNum != 1) {
            ...
            inventory.setItem(AdminGroupUserListGUI.SignEnum.LAST.slot, left);
        }
        if (!Objects.isNull(rightItemMeta) && move != 0 && (pageNum + 1) * VIEW_SIZE < list.size()) {
            ...
            inventory.setItem(AdminGroupUserListGUI.SignEnum.NEXT.slot, right);
        }
        if (!Objects.isNull(midItemMeta)) {
            ...
            inventory.setItem(AdminGroupUserListGUI.SignEnum.PAGE.slot, mid);
        }
        // 设置功能性图标
        setSign(inventory, SignEnum.ADD, SignMaterialEnum.PLAYER, LangReader.get("添加成员"), new ArrayList<>());
        setSign(inventory, AdminGroupUserListGUI.SignEnum.DELETE, SignMaterialEnum.OFF, LangReader.get("删除"), new ArrayList<>());
    }
     ...
     * @param lore
     */
    private static void setSign(Inventory inventory, AdminGroupUserListGUI.SignEnum signEnum, SignMaterialEnum signMaterialEnum
    ...
    public static void turnPage(Inventory inventory, int slot, Player player) {
        try {
            ItemStack itemStack = inventory.getItem(AdminGroupUserListGUI.SignEnum.PAGE.slot);
            String name = itemStack.getItemMeta().getDisplayName();
            int page = Integer.parseInt(name.replace("<", "").replace(">", "").trim());

            if (Objects.equals(slot, AdminGroupUserListGUI.SignEnum.LAST.slot)) {
                if (page < 2) {
                    return;
                }
            ...
            if (Objects.equals(slot, AdminGroupUserListGUI.SignEnum.NEXT.slot)) {
    ...
    public static void setSelectItem(Inventory inventory, int slot, Player player){
        AdminGroupUserListGUI adminGroupUserListGUI = MENU_OPENING.get(player.getUniqueId());
        if(slot >= VIEW_SIZE ){
            adminGroupUserListGUI.selectItem = null;
        }
        ...
        try {
            ...
            adminGroupUserListGUI.selectItem = inventory.getItem(slot).getItemMeta().getDisplayName();
            adminGroupUserListGUI.selectItem = ChatColor.stripColor(adminGroupUserListGUI.selectItem);
    ...
    public enum SignEnum {
        ...
        ADD(49, "添加") {
            ...
            public void deal(Inventory inventory, Player player) {
                ...
                // 一定要注销
                unRegister(player);
                new AdminGroupAddUserWayGUI(player, nowGroup);
            }
        },
        DELETE(53, "删除") {
            @Override
            public void deal(Inventory inventory, Player player) {
                ...
                for(int i = 0; i < nowGroup.getUsers().size();i++){
                    if(StringUtils.equals(nowGroup.getUsers().get(i),select)){
                        nowGroup.getUsers().remove(i);
                        GroupReader.getInstance().addToList(nowGroup);
                        player.closeInventory();
                        unRegister(player);
                        player.sendMessage(LangReader.get("删除成功") + ":" + nowGroup.getName() + "->" + select);
                        return;
                    }
                }
            }
         ...
         */
        public static AdminGroupUserListGUI.SignEnum findMatchedSign(int checkSlot) {
            for (AdminGroupUserListGUI.SignEnum sign : AdminGroupUserListGUI.SignEnum.values()) {

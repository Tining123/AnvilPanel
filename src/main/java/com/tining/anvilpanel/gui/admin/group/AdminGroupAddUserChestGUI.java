...
public class AdminGroupAddUserChestGUI {
    private static final Logger log = Logger.getLogger("Minecraft");
    /**
     * 当前开启的菜单
     */
    private static final Map<UUID, AdminGroupAddUserChestGUI> MENU_OPENING = new ConcurrentHashMap();
     ...
     * @param player
     */
    public AdminGroupAddUserChestGUI(Player player, Group group, List<String> onlineUser) {
    ...
    private static void drawPage(Inventory inventory, int pageNum, Player player) {
        ...
        String selectItem = AdminGroupAddUserChestGUI.MENU_OPENING.get(player.getUniqueId()).selectItem;
        ...
        if (!Objects.isNull(leftItemMeta) && pageNum != 0 && pageNum != 1) {
            ...
            inventory.setItem(AdminGroupAddUserChestGUI.SignEnum.LAST.slot, left);
        }
        if (!Objects.isNull(rightItemMeta) && move != 0 && (pageNum + 1) * VIEW_SIZE < list.size()) {
            ...
            inventory.setItem(AdminGroupAddUserChestGUI.SignEnum.NEXT.slot, right);
        }
        if (!Objects.isNull(midItemMeta)) {
            ...
            inventory.setItem(AdminGroupAddUserChestGUI.SignEnum.PAGE.slot, mid);
        }
        // 设置功能性图标
        setSign(inventory, AdminGroupAddUserChestGUI.SignEnum.ADD, SignMaterialEnum.PLAYER, LangReader.get("添加成员"), new ArrayList<>());
    }
     ...
     * @param lore
     */
    private static void setSign(Inventory inventory, AdminGroupAddUserChestGUI.SignEnum signEnum, SignMaterialEnum signMaterialEnum
    ...
    public static void turnPage(Inventory inventory, int slot, Player player) {
        try {
            ItemStack itemStack = inventory.getItem(AdminGroupAddUserChestGUI.SignEnum.PAGE.slot);
            String name = itemStack.getItemMeta().getDisplayName();
            int page = Integer.parseInt(name.replace("<", "").replace(">", "").trim());

            if (Objects.equals(slot, AdminGroupAddUserChestGUI.SignEnum.LAST.slot)) {
                if (page < 2) {
                    return;
                }
            ...
            if (Objects.equals(slot, AdminGroupAddUserChestGUI.SignEnum.NEXT.slot)) {
    ...
    public static void setSelectItem(Inventory inventory, int slot, Player player){
        AdminGroupAddUserChestGUI AdminGroupAddUserChestGUI = MENU_OPENING.get(player.getUniqueId());
        if(slot >= VIEW_SIZE ){
            AdminGroupAddUserChestGUI.selectItem = null;
        }
        ...
        try {
            ...
            AdminGroupAddUserChestGUI.selectItem = inventory.getItem(slot).getItemMeta().getDisplayName();
            AdminGroupAddUserChestGUI.selectItem = ChatColor.stripColor(AdminGroupAddUserChestGUI.selectItem);
    ...
    public enum SignEnum {
        ...
        ADD(49, "添加") {
            ...
            public void deal(Inventory inventory, Player player) {
                ...
                player.sendMessage(LangReader.get("添加成功") + ":" + user + "->" + group.getName());
            }
        },
        ;
         ...
         */
        public static AdminGroupAddUserChestGUI.SignEnum findMatchedSign(int checkSlot) {
            for (AdminGroupAddUserChestGUI.SignEnum sign : AdminGroupAddUserChestGUI.SignEnum.values()) {

...
public class AdminPanelGroupListGUI {
    private static final Logger log = Logger.getLogger("Minecraft");
    /**
     * 当前开启的菜单
     */
    private static final Map<UUID, AdminPanelGroupListGUI> MENU_OPENING = new ConcurrentHashMap();
     ...
     * 收购列表名称
     */
    public static final String GUI_NAME = "管理员命令用户组列表";
     ...
     * @param player
     */
    public AdminPanelGroupListGUI(Player player, List<Group> groupList, Panel panel) {
    ...
    private static void drawPage(Inventory inventory, int pageNum, Player player) {
        List<Group> groupList = MENU_OPENING.get(player.getUniqueId()).groupList;
        int move = 0;
        boolean set = false;
        ...
        Group selectItem = AdminPanelGroupListGUI.MENU_OPENING.get(player.getUniqueId()).selectItem;
        ...
        if (!Objects.isNull(leftItemMeta) && pageNum != 0 && pageNum != 1) {
            ...
            inventory.setItem(AdminPanelGroupListGUI.SignEnum.LAST.slot, left);
        }
        if (!Objects.isNull(rightItemMeta) && move != 0 && (pageNum + 1) * VIEW_SIZE < list.size()) {
            ...
            inventory.setItem(AdminPanelGroupListGUI.SignEnum.NEXT.slot, right);
        }
        if (!Objects.isNull(midItemMeta)) {
            ...
            inventory.setItem(AdminPanelGroupListGUI.SignEnum.PAGE.slot, mid);
        }
        // 设置功能性图标
        setSign(inventory, AdminPanelGroupListGUI.SignEnum.VIEW, SignMaterialEnum.LABEL, LangReader.get("查看组"), new ArrayList<>());
        setSign(inventory, AdminPanelGroupListGUI.SignEnum.ADD, SignMaterialEnum.ANVIL, LangReader.get("添加组"), new ArrayList<>());
        setSign(inventory, AdminPanelGroupListGUI.SignEnum.DELETE, SignMaterialEnum.OFF, LangReader.get("删除"), new ArrayList<>());
    }
     ...
     * @param lore
     */
    private static void setSign(Inventory inventory, AdminPanelGroupListGUI.SignEnum signEnum, SignMaterialEnum signMaterialEnum
    ...
    public static void turnPage(Inventory inventory, int slot, Player player) {
        try {
            ItemStack itemStack = inventory.getItem(AdminPanelGroupListGUI.SignEnum.PAGE.slot);
            String name = itemStack.getItemMeta().getDisplayName();
            int page = Integer.parseInt(name.replace("<", "").replace(">", "").trim());

            if (Objects.equals(slot, AdminPanelGroupListGUI.SignEnum.LAST.slot)) {
                if (page < 2) {
                    return;
                }
            ...
            if (Objects.equals(slot, AdminPanelGroupListGUI.SignEnum.NEXT.slot)) {
    ...
    public static void setSelectItem(Inventory inventory, int slot, Player player) {
        AdminPanelGroupListGUI adminGroupListGUI = MENU_OPENING.get(player.getUniqueId());
        if (slot >= VIEW_SIZE) {
        ...
        try {
            ...
            adminGroupListGUI.selectItem = MENU_OPENING.get(player.getUniqueId()).groupList.get(index);
    ...
    public enum SignEnum {
        ...
        ADD(50, "添加") {
            ...
            public void deal(Inventory inventory, Player player) {
                ...
                player.closeInventory();
                new AdminPanelGroupAddGUI(player, panel);
            }
        },
        DELETE(53, "删除") {
            @Override
            public void deal(Inventory inventory, Player player) {
                ...
                Group group = MENU_OPENING.get(player.getUniqueId()).selectItem;
                Panel panel = MENU_OPENING.get(player.getUniqueId()).panel;
                player.closeInventory();
                unRegister(player);
                // TODO: 从面板所属删除而不是直接删除
                // new AdminGroupDeleteGUI(player, MENU_OPENING.get(player.getUniqueId()).selectItem);
                new AdminPanelGroupDeleteGUI(player, panel, group.getName());
            }
        };
         ...
         */
        public static AdminPanelGroupListGUI.SignEnum findMatchedSign(int checkSlot) {
            for (AdminPanelGroupListGUI.SignEnum sign : AdminPanelGroupListGUI.SignEnum.values()) {
